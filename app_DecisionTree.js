...
window.nmode = true;
window.editingEnabled = true; //whether nodes/edges can be CREATED
...
window.tool_version = 2
window.run_rules_on_input = false //if false, changes to user input do not trigger rules to run
window.autocomplete_index = {} //will be set to a Bloodhound instance
window.layers_enabled = false // cannot create multiple networks
...
//search index for variables
$(document).ready(function(){
    autocomp_init([]) //starts empty
});

function autocomp_init(names){
	$('.typeahead').typeahead('destroy');
	// Constructing the suggestion engine
	window.autocomplete_index = new Bloodhound({
	...
	{
			source: window.autocomplete_index
	});
}
...
boxSelectionEnabled: false, //turn off click and drag
    ...
    .selector('node:selected')
	  ...
	  .selector('node.resize')
       .css({
		...
		.css({
			content: 'data(conditions_label)', // maps node label to data.label
		'shape': 'square',
		'text-wrap': 'wrap',
		'text-max-width': 200,
		'width': 20,
  ...
  ready: function(){
  	console.log('decisiontree builder app loaded')
    var cy = this;
	//if we have been given a project id, load that project
	if(window.esynOpts.projectid != -1 || window.esynOpts.publishedid != -1){
		if(window.location.href.indexOf('/tool.php?') != -1 ){
      //|| window.location.href.indexOf('/interactive.php?') != -1
			load_from_server(build_app)
		} else {
			load_from_server()
		}
		...
		if(window.esynOpts.query != -1){
			...
			if(window.esynOpts.source == 'biogrid'){
				...
				if(window.esynOpts.query.split("|").length == 1 && window.esynOpts.interInter == "false" && window.esynOpts.identifierType == "id" && window.esynOpts.interactionType != "any"){
					//under these conditions we can do a much faster query
					fb_sql_query(window.esynOpts.query.split("|"), window.esynOpts.interactionType, nwname,window.esynOpts.layout, nwname)
				} else {
...
function init_network_stack(){
	...
	//conditions
	window.stack.metadata.conditions = {} // edge id -> conditions
	//user defined variables
	window.stack.metadata.variables = {} // variable name -> properties
	//user defined groups of variables
	window.stack.metadata.dt_ui_groups = {} // group name -> [variables]
	window.stack.metadata.dt_ui_groups[''] = [] //empty string is always the group when a new variable is created
	window.stack.metadata.dt_rules = {} // functions to run
	window.stack.metadata.dt_description = "" //description of model

	//globals not related to the stack i.e. will never want to save
	window.model_ui = {}
	window.model_ui.dt_edited_inputs = {} //variables in the model that the user has edited
	window.model_ui.dt_calculated_inputs = {} //variables that have been calculated
	window.stack.metadata.dt_version = window.tool_version //version of created decisiontree data. will be reset if data loaded
}
...
cy.on('vclick', 'node', function(evt){
  ...
  var node = evt.target;
	var is_self_loop = node.id() == window.sourcenode;
	//check for a loop via a labelnode
	var has_edge_from = node.outgoers().outgoers();
	var srcn = cy.$('#' + window.sourcenode)
	var is_source_loop = has_edge_from.intersection(srcn).length > 0
	var is_loop = (is_self_loop || is_source_loop)

	//selection of nodes is handled differently by node and edge mode

  console.log( 'tapped ' + node.id() ); //node.id() is a shortcut to get the id
//	console.log('source loop:', is_source_loop, 'self loop:', is_self_loop, 'any loop:', is_loop)
  ...
  if(window.nmode==false && window.currentNwName != 'Merge-result'){
	  if(window.sourcenode != -1 ){
			if(!node.hasClass('labelnode') && !is_loop){
				//this is the second node so draw an edge
				var source = cy.$("#" + window.sourcenode)
				create_edge_with_label(source, node, true)
				clear();
			...
			//cannot create an edge from a label node
			if(!node.hasClass('labelnode')){
				...
				ip_edge_started(window.etype, node.data('name'))
			}
	  }
	  } else{
		...
		//node that is clicked should always become selected
		select_node(node)
		display_info(node)
	}
...
function disselect_all(){
	//disselect anything currently selected
...
//if run_layout == false, the label is created at whatever the mid point
//between the source and target nodes is
function create_edge_with_label(source, target, run_layout){
	//first create the label node
	var srcpos = source.renderedPosition()
	var tgtpos = target.renderedPosition()
	var xPos = (srcpos.x + tgtpos.x) / 2
	var yPos = (srcpos.y + tgtpos.y) / 2
	var label_node_id = "n" + window.stack.metadata['nodecounter']; //also the default id
	var default_marking = 0;
	// console.log(xPos.toString(), yPos.toString())
	add_node_at(xPos, yPos, label_node_id, "", 'labelnode', default_marking)
	window.stack.metadata.conditions[label_node_id] = []

	//from window.sourcenode to labelnode
	...
	//from labelnode to clicked node
	...
	if(run_layout == true){
		position_node_edges(source.id(), label_node_id)
	}

	return label_node_id;
}
	...
	//clear()
  	...
  	if(window.currentNwName == 'Merge-result'){
	  	...
	  	if(node.hasClass('place')){
				ip_node_info(node.id())

	 	} else if (node.hasClass('labelnode')){
			//condition handler

			ip_edge_info(node.id());

			//select all parts of the edge
			var connected = node.connectedEdges()
			var all_parts = node.add(connected)
			all_parts.selectify().select().unselectify()
...
function select_incomers(node_id){
	var node = cy.$('#' + node_id)
	node.incomers().selectify().select().unselectify()
}

function select_outgoers(node_id){
	var node = cy.$('#' + node_id)
	node.outgoers().selectify().select().unselectify()
}

function select_neighborhood(node_id){
	var node = cy.$('#' + node_id)
	node.neighborhood().selectify().select().unselectify()
}
...
//toggle the highlight class for a node
...
function remove_element(){
	...
	//for any normal nodes (places), select their neighborhood to also delete
	//all connected edges via labelnodes
	var selected_places = selected.filter('.place')
	var neighborhood = selected_places.neighborhood()
	...
	for(var i=0; i<selected.length; i++){
		...
		if(el.isNode()){
			...
			//end if place
		} else if(el.hasClass('labelnode')){
				delete_labelnode(el.id())
		} else {
				//transition
			...
			//now handle metadata for affected edges (which will be deleted from graph automatically by cy but metadata must be handled here)
			//get connected edges
			var edges = el.connectedEdges()
			console.log('update metadata for ', edges.length, ' connected edges')
			edges.forEach(function(edg){
				var eid = edg.data('id')
				delete_edge_data(eid)
			})
		} else {
			var eid = el.data('id')
			console.log('element is an edge, id: ', eid)
			delete_edge_data(eid)
		}
	};
...
//handles all metadata updates needed when an edge is deleted
//does not delete the edge itself
function delete_edge_data(eid){
	//remove citation data
	if(window.stack.metadata.citations.hasOwnProperty(eid)){
		console.log('deleting citation data')
		delete window.stack.metadata.citations[eid];
	}

}

function delete_labelnode(eid){
	//handle variables
	var conds = window.stack.metadata.conditions[eid]
	conds.forEach(function(el){
		//need to get the same string as used in the condition list and pass to
		var selected = el.cond_str
		removeCondition(eid, selected)
	})

	//the edge will be deleted so delete conditions metadata
	delete window.stack.metadata.conditions[eid]
...
cy.on('click', function (e,nmode) {
    ...
    if (e.target === cy && window.editingEnabled == true){
    	if( window.currentNwName != 'Merge-result') {
			if (window.nmode == true ){
				...
				var default_marking = 0;
				console.log(xPos.toString(), yPos.toString())
				//xPos, yPos, label_node_id, label_node_name, node_type, default_marking
				add_node_at(xPos, yPos, default_name, default_name, window.ntype, default_marking)
...
//xPos, yPos, label_node_id, label_node_name, node_type, default_marking
function add_node_at(xPos, yPos, node_id, name, node_type, marking){
	cy.add([{
		group : "nodes",
		data : {
			id : node_id,
			name : name,
			marking : marking, //always initialise as 0, transitions will have multiplicity = 0, doesn't matter
			conditions_label: ""
		...
		selectable: false //we want everything unselectified by default
	}]);

	//update counter
	window.stack.metadata['nodecounter'] += 1;

	//update node/place name metadata unless labelnodes which are not tracked
	if(node_type != 'labelnode'){
		window.stack.metadata[node_type][name] = marking;
	}
...
cy.on('tap','edge', function(evt){
	...
	//update info panel
	printEdgeOptions(edge);

});

function printEdgeOptions(edge){
	var id = cy.$('#'+id);

	//if the edge is directed, the source is the labelnode, else the target is the labelnode
	var n;
	if(edge.hasClass('directed')){
		n = edge.source()
	} else {
		//the target is the labelnode
		n = edge.target()
	}
	console.log(n.id() , 'is the labelnode')
	//trigger a vclick event on the labelnode rather than calling display_info directly
	//because the vclick sorts out selectify-ing
	n.trigger('vclick')
};
...
function save(what){
	...
	if(what == 'all'){
		...
		window.stack.metadata.dt_description = $('#de_text').val()
		...
		upload_add_rules(window.stack.metadata.dt_rules)

	}
};

//started making a specific save function but not needed. Functions are just skipped
// function save_metadata(){
// 	var output = {}
// 	var all_props = Object.keys(window.stack.metadata)
// 	var manual = ['dt_rules'] //leys that will not be processed automatically

// 	//specifically handle anything unusual, then stringify the rest
// 	var dt_rules = {}
// 	var rule_keys = Object.keys(window.stack.metadata.dt_rules)
// 	rule_keys.forEach(function(el){
// 		var rdata = {}
// 		var rule = window.stack.metadata.dt_rules[el]
// 		var ks = Object.keys(rule)
// 		ks.forEach(function(outer){
// 			var type = typeof(rule[outer])
// 			if(type == 'function'){
// 				continue
// 			} else if(type == 'object'){
// 				var innerkeys = Object.keys(rule[outer])
// 				innerkeys.forEach(function(inner){
// 					//inner values are not objects
// 					if(type == 'function'){
// 						continue
// 					} else {
// 						rdata[outer][inner] = rule[outer][inner]
// 					}
// 				})
// 			} else {
// 				rdata[outer] = rule[outer]
// 			}
// 		})

// 		dt_rules[el] = rdata
// 	})
// 	output['dt_rules'] = dt_rules

// 	all_props.forEach(function(el){
// 		if(manual.indexOf(el) == -1){
// 			output[el] = window.stack.metadata
// 		}
// 	})
// }
...
function validUpload(){
	//currently no checks on file name
...
function batchListUpload(){
	//process the selected file, attempt to use it to create a stack
	var fileInput = document.getElementById('file_upload_batch');
	var file = fileInput.files[0];
	console.log('processing upload of:' + file.name + ' detected type: ' + file.type);
	var reader = new FileReader();
	reader.onload = function(e){
		...
		if(file.type.match(textType)){
			...
			if(upfile.search(/\n/) < 0){ //if there are no \n characters, convert to \n
				var upfile = upfile.replace(/\r/g, "\n");
			}
			var names = upfile.split('\n')
			var to_update = cy.filter(function(element, i, eles){
			  if( element.isNode() && names.indexOf(element.data('name')) != -1 ){
			    return true;
			  }
			  return false;
			});
			console.log(to_update)
			var action = $("input:radio[name='batchradio']:checked").val();
			if(action == "add_highlight"){
				to_update.addClass("highlight")
			} else if(action == "remove_highlight"){
				to_update.removeClass("highlight")
			}
		}
	}
	reader.readAsText(file);
...
function uploadFromEsyn(uploaded_json){
	...
	//rebuild rules
	upload_check_version()
	upload_add_rules(window.stack.metadata.dt_rules)
	autocomp_init(_.keys(window.stack.metadata.variables))
	...
	var autoselect = nn[0]
	cy.add(JSON.parse(window.stack[autoselect])) //load and display the network
	...
	re_refresh()
	ve_refresh()
//	_run_dt_fixes() //fixes break the new conditions structure

	//show the model description
	$('#de_text').val(window.stack.metadata.dt_description)
...
//uploading a csv of data to run in batch mode
function uploadBatchRun(){
	$('#file-upload-btn-batch').prop('disabled','disabled')
	clearBatchRunResults()
	//process the selected file, attempt to use it to create a stack
	var fileInput = document.getElementById('file_upload_batch_run');
	var file = fileInput.files[0];
	//window.batch_file = file
	console.log('processing upload of:' + file.name + ' detected type: ' + file.type);

	window.batch_mode_dataset = new Dataset()

	var reader = new FileReader();
	reader.onload = function(e){
		var textType = /text.*/
		//this was needed to get office365 csv files to upload
		var excelcsvType = /vnd.ms-excel/
		if(file.type.match(textType) || file.type.match(excelcsvType)){
			var upfile = reader.result
			//there is an issue with newlines not being included, add them back
			//might be an effect of OS/ browser/ excel/ all of these
			if(upfile.search(/\n/) < 0){ //if there are no \n characters, convert to \n
				upfile = upfile.replace(/\r/g, "\n");
			}
			//var newlines = upfile.split('\n')
			var res = $.csv.toArrays(upfile); //try with a comma separator
			console.log(res)

			//the first line is the header
			var colnames = res[0]

			runBatchAndSave(colnames, res)


		} else {
			$('#dt-model-result-batch').text('The selected file is not compatible with esyN. You can only upload a csv file.')
		}
	}
	reader.readAsText(file);


	$('#file-upload-btn-batch').prop('disabled',false);

}

function runBatchAndSave(colnames, res){
	$('#dt-model-result-batch').empty().html("<h4>Results</h4>")
	var results = []
	var bigbatch = res.length > 100
	if(bigbatch){
		console.log("large batch, Results will not be printed.")
		var head_div = "batch_header_row_" + i
		var newdiv = $( "<div id='"+head_div+"'></div>" )
		$("#dt-model-result-batch").append(newdiv)
		$('#' + head_div).html("<br /> <b>Batch too large to print. Results can be downloaded when done.</b>")
		console.log("disabling console for remainder of session.")
		console.log = function(){}
	}

	for (var i = 1; i < res.length; i++) {
		window.model_eval_log = []

		var params = _.object(colnames, res[i])
		params = batchSetTypes(params)
		params_raw = _.clone(params)

		//run
		//checks on structure of the graph
		var root_nodes = cy.nodes().roots()
		if(root_nodes.length == 0){
			alert("A decision tree must have a root node, none detected")
			return
		}
		if(root_nodes.length > 1){
			alert("A decision tree must have exactly one root note, multiple detected")
			return
		}
		var root_node = root_nodes[0]
		console.log("detected root node", root_node)
		...
		rules.forEach(function(el){
		    then_action = window.stack.metadata.dt_rules[el].then_str
		...
		//find all missing variables first so we know which can be overwritten
		var missing = []
		var required = []
		var all_vars = Object.keys(window.stack.metadata.variables)
		all_vars.forEach(function(el){
			if( window.stack.metadata.variables[el].required == true ){
				required.push(el)
			}
			var val = params[el]
			var type = window.stack.metadata.variables[el].value_type
			//added extra val=="NaN" result seen in excel upload
			if ( type == 'num' && isNaN(val) || type == 'bool' && val === '' || type == 'str' && val == '' || val == 'NaN'){
				missing.push(el)
			}
		})

		var calculated = run_calculators(set_val_rules, params)
		var calc_vars = _.keys(calculated)
		var pos;
		calc_vars.forEach(function(cv){
			pos = missing.indexOf(cv)
			if(pos != -1){
				params[cv] = calculated[cv]
				missing.splice(pos, 1)
			}
		})
		//check for intersection of missing variables and required variables
		missing = _.intersection(missing, required)

		//now run not-valid rules using updated input
		//if any rules run, set can_run_model to false
		var not_valid_rules_state = {}
		var val_rule_log = []
		can_run_model = true
		not_valid_rules.forEach(function(el){
		    var do_then = window.stack.metadata.dt_rules[el].if(params)
		    if(do_then == 1){
		        can_run_model = false
		        not_valid_rules_state[el] = true
		        val_rule_log.push('Failed input check rule: ' + el)
		    } else {
		        not_valid_rules_state[el] = false
		    }
		})

	//check required values still missing after running calculators
	if ( missing.length != 0 || !can_run_model ) {
		window.model_eval_log.push('Stopped: model input not valid')
		var result = {reachable_ids:[], next_node:root_node, path_nodes:[''], path_edges:[''], esyn_result:"", result_is_leaf:""}
	} else {
		//run model
		var result = traverse_from(root_node, params)
		results.push(result)
	}

	//update UI
	if(bigbatch){
		var node_res = show_dt_result(result.reachable_ids, result.next_node, undefined, [[]], []) //[[]] for per step, [] for required. Currently not shown.
	} else {
		var head_div = "batch_header_row_" + i
		var newdiv = $( "<div id='"+head_div+"'></div>" )
		$("#dt-model-result-batch").append(newdiv)
		var ds_p = i - 1 //dataset index starts at 0
		var head_txt = "<br /> <b>Row " + i + "</b> "
		head_txt += '<button class="btn btn-info btn-sm" onclick="show_dataset_path(window.batch_mode_dataset, '+ds_p+')">show path</button>'
		$('#' + head_div).html(head_txt)
		var res_div = "batch_row_" + i
		var newdiv = $( "<div id='"+res_div+"'></div>" )
		$("#dt-model-result-batch").append(newdiv)
		var node_res = show_dt_result(result.reachable_ids, result.next_node, res_div, [[]], []) //[[]] for per step, [] for required. Currently not shown.
	}

	result['esyn_result'] = node_res['conclusion']
	result['esyn_result_is_leaf'] = node_res['leaf_reached']
	result['esyn_required_variables_present'] = missing.length == 0
	result['esyn_input_rules_passed'] = can_run_model
	result['esyn_list_missing_required'] = missing.toString()
	result['esyn_list_failed_input_rules'] = val_rule_log.join("|")

	//dataset
	var result_ds = {'output': result, 'input': params_raw, 'model_input': params }
	result_ds['meta'] = {}
	result_ds['meta']['id']
	window.batch_mode_dataset.set_and_move(result_ds , 'next')
	}
}

function clearBatchRunResults(){
	$('#dt-model-result-batch').empty()
}

function downloadBatchRunResults(inc_calc){
	var meta_cols = ["esyn_result", "esyn_result_is_leaf", "esyn_required_variables_present", "esyn_input_rules_passed", "esyn_list_missing_required"]
	dataset_to_csv(window.batch_mode_dataset, 'batch_results.csv', meta_cols, inc_calc)
}

function show_dataset_path(dataset, pos){
	resetAllSettings()
	result = dataset.data[pos].output
	show_dt_path(result.path_nodes, result.path_edges)
}

//tried to develop a coverage test, not sure it is completely accurate but ok
function show_batch_coverage(){
	resetAllSettings()
	//collect nodes and edges to highlight
	var nodes = {}
	var edges = {}
	window.batch_mode_dataset.data.forEach(function(el){
		if(el.output.hasOwnProperty("path_nodes")){
			el.output.path_nodes.forEach(function(n){
			...
			el.output.path_edges.forEach(function(n){
	...
	show_dt_path(_.keys(nodes), _.keys(edges))
	var n_e_covered = _.keys(edges).length
	var n_e_total = cy.edges().length
	var pr_e = n_e_covered / n_e_total
	console.log("edges covered", n_e_covered, "/", n_e_total, "(", pr_e,")")

	//nodes covered
	var n_n_covered = _.keys(nodes).length
	var n_n_total = cy.nodes().length - cy.nodes(".labelnode").length //don't count labels
	var pr_n = n_n_covered / n_n_total
	console.log("nodes covered", n_n_covered, "/", n_n_total, "(", pr_n,")")

	//excluding leaves
	var n_n_noleaf = n_n_total - cy.nodes().leaves().length
	var pr_n_ex = n_n_covered / n_n_
	console.log("nodes covered", n_n_covered, "/", n_n_total, "(", pr_n,")")
}

function batchSetTypes(params){
	var tr = get_true_opts()
	var fl = get_false_opts()
	var all_vars = Object.keys(window.stack.metadata.variables)
	all_vars.forEach(function(el){
		var val = params[el]
		var type = window.stack.metadata.variables[el].value_type
		if(type == "num"){
			params[el] = parseFloat(val)
		}
		if(type == "bool"){
			if(tr.indexOf(val) != -1){
				params[el] = true
...
function upload_add_rules(rules){
...
//check that the loaded version is consistent with the current version of the app
function upload_check_version(){
	var lv = window.stack.metadata.dt_version
	var loaded_version = (typeof(lv) == 'undefined') ? 0 : lv
	if(window.tool_version > loaded_version){
		console.log('version update required')
		incremental_upgrade(loaded_version)
	} else {
		console.log('version is compatible')
	}
}

//apply changes to go from one version to another starting from given point
function incremental_upgrade(start_version){
	var v = start_version
	while(v < window.tool_version){
		if(v == 0){
			//upgrade from 0 to 1
			console.log('update v0 to v1')
			window.stack.metadata.dt_rules = {}
			window.stack.metadata.dt_ui_groups = {}
			window.stack.metadata.dt_ui_groups[''] = []
			window.stack.metadata.conditions = {}
			window.stack.metadata.variables = {}

			var nn = Object.keys(window.stack);
			nn.splice(nn.indexOf('metadata'),1)
			nn.forEach(function(netname){
				cy.add(JSON.parse(window.stack[netname]))
				//iterate over edges, add conditions which automatically adds variables
				var edges = cy.edges()
				edges.forEach(function(el){
					var edata = el.data()
					var conds = Object.keys(edata.conditions)
					var id = edata.id
					window.stack.metadata.conditions[id] = []
					conds.forEach(function(c){
						var condition_obj = {}
						var co = edata.conditions[c]
					    co['value_type'] = co['value type']
					    console.log('adding ', co, 'to ', id)
					    addCondition(co, id)
	    				updateConditionLabel(id)
		...
		if(v == 1){
			//upgrade to version with labelnodes
			var nn = Object.keys(window.stack);
			nn.splice(nn.indexOf('metadata'),1)
			nn.forEach(function(netname){
				cy.add(JSON.parse(window.stack[netname]))
				...
				var edges = cy.edges()
				edges.forEach(function(el){
					var old_edge_id = el.id()
					var source = el.source()
					var target = el.target()

					// create new labelnode edge
					var label_id = create_edge_with_label(source, target, false)

					//add all conditions to the labelnode
					if(window.stack.metadata.conditions.hasOwnProperty(old_edge_id)){
						//don't just do this
						//window.stack.metadata.conditions[label_id] = window.stack.metadata.conditions[old_edge_id]
						//because we need to also update variable appearances
						var conds = window.stack.metadata.conditions[old_edge_id]
						conds.forEach(function(c){
							addCondition(c, label_id)
						})
					}
					updateConditionLabel(label_id)

					//delete metadata for the edge to sort out appearances
					console.log('deleting old label', old_edge_id)
					delete_labelnode(old_edge_id)
				...
				cy.nodes().unselectify()
				cy.edges().unselectify()
				cy.layout({name:'dagre',nodeDimensionsIncludeLabels: true,
					nodeSep:15, ranker:'tight-tree'
				}).run()
				//update the stack - can't use updatestack as that uses window.currentnwname
				var jdata = cy.json()['elements'];
				if('nodes' in jdata){ //it breaks if you can save an empty network to the stack
					window.stack[netname] = JSON.stringify(cy.json()['elements']);
				}
				cy.elements().remove()
	...
	window.stack.metadata.dt_version = v
}


//export model for python library to run
//currently only supports exporting the currently loaded network
function exportDecisionTree(){
	var output = get_all_conditions()
	console.log(output)
	var out_s = JSON.stringify(output)
	var blob = new Blob([out_s], {type: "text/plain;charset=utf-8"});
	saveAs(blob, "decisiontree.txt");

}

function get_all_conditions(){
	//iterate over all edges and build output
	var output = []
	var cond_nodes = cy.nodes(".labelnode")
	var current_group = 1
	cond_nodes.forEach(function(el){
		var eid = el.id()
		window.stack.metadata.conditions[eid].forEach(function(cond){
			var row = cond
			row['group'] = current_group
			row['source'] = el.incomers("node").data('name')
			row['target'] = el.outgoers("node").data('name')
			output.push(row)
		})
		current_group++
...
function getPropsInUserOrder(){
	//get all unique properties from the model
	var conds = get_all_conditions()
	var all_props_obj = {}
	conds.forEach(function(el){
		if(el.operation != 'else'){
			all_props_obj[el.prop] = el
		}
	})

	var cond_strs = []
	//get conditions in the order of groups if groups set
	var all_groups = _.keys(window.stack.metadata.dt_ui_groups)
	all_groups.forEach(function(gr){
		//add each prop
		var all_props = window.stack.metadata.dt_ui_groups[gr].sort().slice() //copy the array
		if(all_props.indexOf("") != -1){
			all_props.splice(all_props.indexOf(''),1) //else conditions create an empty string prop, skip it
		}
		all_props.forEach(function(el){
			var prop_data = all_props_obj[el]
			//check for undefined is a fix for projects created while condition deletion was buggy
...
//export all patient properties used in model
function exportDecisionTreeProps(){
	var cond_strs = getPropsInUserOrder()
	var header = '"' + cond_strs.join('","') + '"\r\n'
	var blob = new Blob([header], {type: "text/plain;charset=utf-8"});
	saveAs(blob, 'model_variables.csv');
}

function build_app(with_header=true, interactive=false){
	//get all unique properties from the model
	var conds = get_all_conditions()

	//create/ reset the container table
	init_model_input()

	var tbl = document.getElementById('dt-model-params')
	var all_groups = _.keys(window.stack.metadata.dt_ui_groups)
	all_groups.forEach(function(gr){
		//make a heading row
    if(with_header == true){
      var row = tbl.insertRow(-1) //position 0 will insert above the header
      row.innerHTML = '<b>'+gr+'</b>'
    }
		//add each prop
		var all_props = window.stack.metadata.dt_ui_groups[gr].sort().slice() //copy the array
		if(all_props.indexOf("") != -1){
		...
		all_props.forEach(function(el){
			//console.log(el)
			var prop_data = window.stack.metadata.variables[el]
			//console.log('build_app pr_data ', prop_data)
			//check for undefined is a fix for projects created while condition deletion was buggy
			if(typeof(prop_data) == 'undefined'){
				console.log('no data for prop', el)
			} else {
				add_model_input(el, prop_data)
			}
		})
	})

	//global tracker for variables the user has set
	window.model_ui.dt_edited_inputs = {}
	window.model_ui.dt_calculated_inputs = {}
  window.can_calculate = get_calculator_targets()

	//set default values
	var user_input = get_model_input()
	var calc = run_all_calculators(user_input)
	var updatelog = update_model_input(calc)

  if(interactive==true){
    run_dt_model('interactive', false, false)
  }

}

function get_calculator_targets(){
  var rules = get_rules_by_type()
  var can_set = {}
  var k = []
  var r, rv1, rv2, sets;
  rules.set_val_rules.forEach(function(el){
    r = window.stack.metadata.dt_rules[el]
    sets = r.then.set_variable
    if(!can_set.hasOwnProperty(sets)){
      can_set[sets] = {'inputs':[], 'rules':[]}
      k.push(sets)
    }
    rv1 = ts_find_vars(r.if_str)
    rv2 = ts_find_vars(r.then.str)
    can_set[sets]['inputs'] = can_set[sets]['inputs'].concat(rv1)
    can_set[sets]['inputs'] = can_set[sets]['inputs'].concat(rv2)
    can_set[sets]['rules'].push(el)
  })
  //for each variable get the unique inputs that can be used
  k.forEach(function(el){
    can_set[el]['inputs'] = _.uniq(can_set[el]['inputs'])
  })
  return(can_set)
}

function init_model_input(){
	var tbl = ''
	tbl += '<thead><tr><th class="col-varname">Variable</th><th class="col-input">Value</th><th>Calculated</th></tr></thead><tbody></tbody>'
	$('#dt-model-params').html(tbl)
}

function add_model_input(pr_name, pr_data){
	var tbl = document.getElementById('dt-model-params')
	var row = tbl.getElementsByTagName('tbody')[0].insertRow(-1) //position 0 will insert above the header
  row.id = "mir_" + pr_name //mir - model input row
	var c0 = row.insertCell(0)
	//console.log(pr_data)
	var txt = pr_data.required ? "*" : "&nbsp;"
  txt += pr_name
  if(pr_data.hasOwnProperty('description')){
    txt += "<br/><small>" + pr_data.description + "</small>"
  }
	c0.innerHTML = txt

	var vtype = pr_data['value_type']
  var p;
	var cell_html;
	//triggers can be oninput to evaluate per keystroke but creates problems e.g. if rule triggers at 12 and user is typing 120, rule will run after 12 and could set various values
	if(vtype == "num"){
    p = "Number"
		cell_html = "<input type='number' step='0.01' onchange='model_input_changed(this)' name='"+pr_name+"' placeholder='"+p+"'>"
	} else if(vtype == "str"){
    p = "Text"
		cell_html = "<input type='text' onchange='model_input_changed(this)' name='"+pr_name+"' placeholder='"+p+"'>"
	} else if(vtype == "bool"){
		//cell_html = "<input type='checkbox' name='"+pr_data.prop+"'>"
		var t_id = pr_name+"_true"
		var f_id = pr_name+"_false"
		var u_id = pr_name+"_unknown"
		cell_html = '<input class="form-check-input" onchange="model_input_changed(this)" type="radio" name="'+pr_name+'" id="'+t_id+'" value="True">'
        cell_html += '<label class="form-check-label" for="'+t_id+'">True</label>'
        cell_html += '<input class="form-check-input" onchange="model_input_changed(this)" type="radio" name="'+pr_name+'" id="'+f_id+'" value="False">'
        cell_html += '<label class="form-check-label" for="'+f_id+'">False</label>'
        cell_html += '<input class="form-check-input" onchange="model_input_changed(this)" type="radio" name="'+pr_name+'" id="'+u_id+'" value="Unknown" checked>'
        cell_html += '<label class="form-check-label" for="'+u_id+'">Unknown</label>'

	}
	var c2 = row.insertCell(1)
	c2.innerHTML = cell_html

  var c3 = row.insertCell(2)
  c3.innerHTML = "<p id='calc_val_" + pr_name + "'></p>"
}

//returns
/*
'output': result from the tree,
'input': unmodified user input from the ui,
'model_input': user input with calculators applied}
*/
function run_dt_model(missing_action = 'highlight', use_calculators = true, enforce_required = true){
	var can_run = true;
	window.model_eval_log = []
	//clear_dt_path()
	clear_dt_results()
	...
	var user_raw_input = get_model_input()

	var rules = run_rules(user_raw_input) // updated user input is in rules.updated_input
	window.model_eval_log = window.model_eval_log.concat(rules.log)
  ...
  if(enforce_required == true){
    var all_vars = Object.keys(model_input)
    all_vars.forEach(function(el){
      if( window.stack.metadata.variables[el].required == true ){
        var val = model_input[el]
        var type = window.stack.metadata.variables[el].value_type
        if ( type == 'num' && isNaN(val) || type == 'bool' && val === '' || type == 'str' && val == ''){
          missing.push(el)
          window.model_eval_log.push("Missing required input: " + el)
        }
      }
    })
  }
  ...
  if ( missing.length != 0 || !rules.can_run ) {
    window.model_eval_log.push('Stopped: model input not valid')
    show_dt_log()
    var node_res = show_dt_result([], '', 'dt-model-result', [[]], missing, missing_action)
    can_run = false
  }
	...
	if(can_run){
		...
		//run model
		result = traverse_from(root_node, model_input)
		var node_res = show_dt_result(result.reachable_ids, result.next_node, 'dt-model-result', result.path_missing, [], missing_action)
		show_dt_log()
		show_dt_path(result.path_nodes, result.path_edges)
		result['esyn_result'] = node_res['conclusion']
		result['esyn_result_is_leaf'] = node_res['leaf_reached']
	} else {
		result = {reachable_ids:[], next_node:'', path_nodes:[''], path_edges:[''], esyn_result:"", result_is_leaf:""}
	}

	return {'output': result, 'input': user_raw_input, 'model_input': model_input}
}

function traverse_from(root_node, user_input){
	var options = get_targets_from(root_node)
	var can_reach = conditions_met(root_node, options, user_input)
  var missing_per_step = []
	console.log(can_reach)
	var reachable_ids = _.keys(can_reach.reachable_ids)
  missing_per_step.push(can_reach.missing)
	var path_nodes = [root_node.id()]
	var path_edges = []
	next_node = root_node //needed in case evaluation stops at root
	while(reachable_ids.length == 1){
		var next_src = reachable_ids[0]
		console.log("continue to", next_src)
		var next_node = cy.$('#' + next_src)
		path_nodes.push(next_src)
		path_edges = path_edges.concat(can_reach[next_src])
		options = get_targets_from(next_node)

		can_reach = conditions_met(next_node, options, user_input)
		console.log(can_reach)
		var reachable_ids = _.keys(can_reach.reachable_ids)
    missing_per_step.push(can_reach.missing)
	}
	var res = {reachable_ids:reachable_ids, next_node:next_node, path_nodes:path_nodes, path_edges:path_edges, path_missing: missing_per_step}
	return res
}

function show_dt_result(reachable_ids, current_node, container_id, missing_per_step, missing_required, missing_action){
	//if container_id is undefined the result is just returned
	var res = ''
	var leaf = false
  var name = "initialisation step"
  if(typeof(current_node) == "object"){
      name = current_node.data('name')
      leaf = current_node.leaves().length == 1
  }
  var hint = ""

  var missing_last = _.uniq(missing_per_step[missing_per_step.length - 1])
  missing_required = _.uniq(missing_required)

  if(missing_required.length != 0){
    res += '<div class="alert alert-danger" role="alert">Stopped: input not valid. Required variables missing.</div>'
    res += "<br/>"
    res += missing_required.join("<br/>")

  }

	if(reachable_ids.length == 0){
		if(leaf){
			console.log("evaluation ended at leaf node")
			res += '<div class="alert alert-success" role="alert">Model result: ' + name + '</div>'
		} else {
			console.log("no options from non-leaf node")
			res += '<div class="alert alert-warning" role="alert">Model halted at ' + name + " because no further conditions met.</div>"
      if(hint != ""){
        res += "<br/>Hint: " + hint
      }
		}
	} else if(reachable_ids.length > 1){
		console.log("too many options possible")
		res += '<div class="alert alert-warning" role="alert">Model halted at ' + name + " because multiple conditions met.</div>"
    if(hint != ""){
      res += "<br/>Hint: " + hint
    }
	}

  if(missing_last.length != 0){
    res += "One or more of the following variables are needed to continue from the current node:"
    res += "<br/>"
    res += missing_last.join("<br/>")

  }
  var all_missing = _.uniq(_.flatten(missing_per_step))
  var other_missing = _.difference(all_missing, missing_last)
  if(other_missing.length != 0){
    res += "Other missing variables that could affect the result:<br/>"
    res += other_missing.join("<br/>")
  }
  ...
  //missing action
  if(missing_action == "highlight"){
    highlight_ui_inputs(missing_required, 'add')
    highlight_ui_inputs(missing_last, 'add')
  }
  if(missing_action == "interactive"){
    var to_show = _.union(missing_required, missing_last)
    console.log("showing",to_show,"from required",missing_required, "last", missing_last)
    update_ui_interactive(to_show)
  }
	return {'conclusion': name, 'leaf_reached': leaf, 'hint': hint}
}

function highlight_ui_inputs(varnames, action){
  var r;
  if(action == "add"){
    varnames.forEach(function(el){
      r = document.getElementById('mir_' + el)
      if(r !== null){
...
function update_ui_interactive(missing){
  console.log("trying to show",missing)
  //missing is the names of variables needed
  //check whether any of these can be calculated
  //... TODO

  //filter UI table
  var all_vars = Object.keys(window.stack.metadata.variables)
  //SHOW everything IN the list
  //HIDE everything NOT IN the list
  all_vars.forEach(function(el){
    if(el != ""){
      var d = document.getElementById("mir_" + el)
      if(missing.indexOf(el) == -1){
...
function show_dt_log(log){
	if (log === undefined) {
		log = window.model_eval_log
	}
	log_el = document.getElementById('dt-model-log')
	log_el.innerHTML = ""
	log = _.unique(log)
...
function show_dt_path(node_ids, edge_ids){
	node_ids.forEach(function(el){
		cy.$('#' + el).addClass('inPath')
	})
	edge_ids.forEach(function(el){
		cy.$('#' + el).addClass('inPath')
	})
}

function clear_dt_path(){
	cy.elements().removeClass('inPath')
}

function show_paths_from_root(){
	cy.elements().removeClass('inPath')
	cy.$(':selected').predecessors().addClass('inPath')
}

function clear_dt_results(){
	clear_dt_path()
	window.model_eval_log = []
	show_dt_log()
	$('#dt-model-result').text('')
  highlight_ui_inputs(Object.keys(window.stack.metadata.variables), 'remove')
}

function get_targets_from(node){
	out_nodes = node.outgoers('node')
	return out_nodes
}

function conditions_met(source, targets, model_input){
  ...
  var missing = []
	for (var i = 0; i < targets.length; i++) {
		var tgt_id = targets[i].id()
		if(window.stack.metadata.conditions.hasOwnProperty(tgt_id)){
			//there are conditions, test them
			var conds = window.stack.metadata.conditions[tgt_id]
			if(conds.length == 1 && conds[0].operation == 'else') {
				//get all edges connected to the labelnode
				var edges = targets[i].connectedEdges()
				var edge_ids = []
				...
				var edge_target = targets[i].outgoers().filter('node')
				var tgt_id = edge_target.id()
				else_options[tgt_id] = edge_ids
				continue
			}
			var edge_conds_met = true
			for (var c = 0; c < conds.length; c++) {
				console.log(conds[c].cond_str)
				var res = evaluate(conds[c], model_input)
				console.log(res)
				...
				if(res.tested == false){
					...
					window.model_eval_log.push(res.reason)
          missing.push(res.missing)
			...
			if(!source.hasClass('labelnode')){
				window.model_eval_log.push("Warning: No conditions for edge")
			}
		}
		if(edge_conds_met){
			//get all edges connected to the labelnode
  ...
  var result = {'reachable_ids': undefined, 'missing': missing}
...
function evaluate(condition, model_input){
	var result = {'result': false, 'reason': 'valid', 'tested' : true, 'missing': undefined}
	if(condition.operation == 'else'){
		result.result = true
		return result
	}
	var mv = model_input[condition.prop]
...
function get_model_input(){
	var inputs = document.getElementById('dt-model-container').getElementsByTagName('input')
	var params = {}
	var radio_names = {}
	for (var i = 0; i < inputs.length; i++) {
		var pr_name = inputs[i].name
		if(inputs[i].type == "radio"){
			radio_names[inputs[i].name] = true
		} else if(inputs[i].type == "number"){
			params[pr_name] = parseFloat(inputs[i].value)
		} else {
			params[pr_name] = inputs[i].value
		}
	}
	//process radio groups
	var r_names = _.keys(radio_names)
	r_names.forEach(function(el){
		var filter = 'input[name="'+el+'"]:checked'
		var rdo_val = $(filter).val()
		if (rdo_val == "True") {
			params[el] = true
		} else if(rdo_val == "False"){
			params[el] = false
		} else {
			params[el] = ""
		}
...
//change model input based on user-defined rules
//this function handles the variable types not the calculation of new values
function update_model_input(updates){
	var to_change = Object.keys(updates)
	var vtype
	var log = []
	to_change.forEach(function(el){
		if(window.model_ui.dt_edited_inputs.hasOwnProperty(el)){
			console.log("variable",el,"has been edited by the user, not changing")
			return
		}
		//track that this value has been set by a calculator
		window.model_ui.dt_calculated_inputs[el] = true
		vtype = window.stack.metadata.variables[el].value_type
		if(vtype == "bool"){
			console.log('updates[el] is', updates[el])
			if(updates[el] == 1){
				$('input[name="'+el+'"][value="True"]').prop('checked', true)
			} else if(updates[el] === ''){
				$('input[name="'+el+'"][value="Unknown"]').prop('checked', true)
			} else {
				$('input[name="'+el+'"][value="False"]').prop('checked', true)
			}
		} else {
			//for str and num can just set val of element
			//for num, check the calculator is returning a num
			if(vtype == 'num' && isNaN(parseFloat(updates[el]))){
				if(updates[el] == ''){ //allow empty string to clear input
					$('input[name="'+el+'"]').val(updates[el])
				} else {
					log.push('Error: a calculator tried to set num variable ' + el + ' to a string')
					console.log('a calculator tried to set num variable', el, 'to a string')
				}
			} else {
				$('input[name="'+el+'"]').val(updates[el])
			}
...
function show_calculated_inputs(updates){
  var to_change = Object.keys(updates)
  var vtype
  var log = []
  to_change.forEach(function(el){
    ...
    //window.model_ui.dt_calculated_inputs[el] = true
    vtype = window.stack.metadata.variables[el].value_type
    var container = document.getElementById("calc_val_" + el)
    if(vtype == "bool"){
      console.log('updates[el] is', updates[el])
      if(updates[el] == 1){
        container.innerHTML = "True"
      } else if(updates[el] === ''){
        container.innerHTML = "Unknown"
      } else {
      ...
      if(vtype == 'num' && isNaN(parseFloat(updates[el]))){
        ...
        container.innerHTML = updates[el].toString()
      }
    }
  })
  return log
...
function clear_calculated_input(varnames){
  varnames.forEach(function(el){
    if(el != ""){
      var container = document.getElementById("calc_val_" + el)
      container.innerHTML = ""
...
function model_input_changed(e){
	//track that the user has edited the value
	window.model_ui.dt_edited_inputs[e.name] = true
	//if this value was previously set by a calculator, delete that flag
	if(window.model_ui.dt_calculated_inputs.hasOwnProperty(e.name)){
		delete window.model_ui.dt_calculated_inputs[e.name]
	}
	if(window.run_rules_on_input){
		//clear any calculated values
		var calculated = Object.keys(window.model_ui.dt_calculated_inputs)
		var clear = {}
		calculated.forEach(function(el){
			clear[el] = ''
		})
		update_model_input(clear)

		var user_input = get_model_input()
		var calc = run_all_calculators(user_input)
		//var update = update_model_input(calc)
    var update = show_calculated_inputs(calc)
		show_dt_log(update.log)
	}

  //need to clear values
  var user_input = get_model_input()
  var all_vars = Object.keys(window.stack.metadata.variables)
  clear_calculated_input(all_vars)
  var calc = run_all_calculators(user_input)
  var update = show_calculated_inputs(calc)
}


function highlight_leaf_nodes(){
	cy.nodes().leaves().addClass('highlight')
}
...
function exportToTC(){
  ...
  //get label nodes
  var n = cy.nodes('.labelnode')

  //iterate over labelnodes getting source and target
  var source, target, cond, label;
  var rows = '"source","target","conditions",\r\n';
  n.forEach(function(el){
    source = el.incomers('node').data('name');
    target = el.outgoers('node').data('name');
    cond = window.stack.metadata.conditions[el.id()]
    label = ""
    cond.forEach(function(c){
        label = label + "(" + c.cond_str + ") AND "
    })
    label = label.slice(0, -5) // remove trailing chars
    //rows.push({'s': source, 't': target, 'c': label})
...
function newnw(){
	if(window.layers_enabled){
		...
		alert("Decision Trees currently do not support multiple networks.")
	}
}
...
//deletes everything in the current cy view
function load_from_stack(nwname){
	...
	//load the pre-existing network that was selected
		cy.elements().remove()
		cy.add(JSON.parse(window.stack[nwname])) //load and display the network
		cy.layout({name:'preset'})
		cy.fit()
...
function remove_style(){
	var toupdate = cy.filter(function(element, i, eles){
...
//remove node highlight from all
...
function megamerge(){
	alert('merging networks is currently disabled')
	return
	/*
	...
	*/
	...
	//this block edited for cy update but not tested
	cy.elements().remove()
	cy.add({
			nodes: mres.nodes,
			edges: mres.edges
...
//if the current network is the only network then this is effectively
//"clear current network" because the current network name is not changed
function remove_network(){
	...
	if(sure == true){
		...
		if(nn.length > 0){
			...
			console.log("no other network to load")
		}
...
function save_to_server(ignore_pid){
	...
	if(dosave){
		...
		window.stack.metadata = JSON.parse(window.stack.metadata);
		upload_add_rules(window.stack.metadata.dt_rules) // rebuild rules
...
function load_from_server(callback){ //add history id
	//Ajax request
		...
		$.ajax({ url: '../manager.php',
	         ...
	         success: function(uploaded_json) {
				...
				if(result.hasOwnProperty('success')){
					...
					re_refresh()
					ve_refresh()
					//_run_dt_fixes() //fixes don't work with the new structure
...
//////////////////// tracking results
function start_tracking_mode(){
	tracking_reset()
	document.getElementById('tracking_tools').style.display = "block"
	build_app()
}

function tracking_next(direction){
	var res = run_dt_model()
	res['meta'] = {}
	res['meta']['id'] = $("#tracking_manual_id").val()
	var next = window.dataset.set_and_move(res, direction)
	tracking_ui_update(next, window.dataset.position)
}

function tracking_reset(){
	window.dataset = new Dataset()
	$('#tracking_internal_id').text('Current row: 1')
	$('#tracking_total_rows').text('Total rows: 1')

}

function tracking_delete_current(){
	var next = window.dataset.delete_and_move();
	tracking_ui_update(next, window.dataset.position)
}

function tracking_ui_update(next, pos){
	build_app()
	update_model_input(next['input'])
	//for now disable because results are inconsistent, probably a race somewhere
	clear_dt_results()
	clear_dt_path()
	var display_pos = pos + 1
	$('#tracking_internal_id').text('Current row: ' + display_pos.toString())
	$('#tracking_total_rows').text('Total rows: ' + window.dataset.data.length.toString())
	$("#tracking_manual_id").val(next['meta']['id'])
}

function tracking_download(){
	//only download esyn_result from esyn metadata
	//other metadata cols not created in dataset mode but are in batch mode
	dataset_to_csv(window.dataset, 'dataset.csv', ['esyn_result'], false);
}

function dataset_to_csv(dataset, fname, esyn_meta_cols, include_calculated){
	var rows = []
	var header_start = ['manual_identifer']
	var input_type = 'input' //raw user input to model, not including calculated values
	if(include_calculated == true){
		input_type = 'model_input'
	}
	//var input_header = Object.keys(dataset.data[0].input)
	var input_header = getPropsInUserOrder()
	var header_arr = header_start.concat(input_header)
	header_arr = header_arr.concat(esyn_meta_cols)
	header = '"' + header_arr.join('","') + '"\r\n'
	rows.push(header)
	var rd, val;
	dataset.data.forEach(function(el){
		if(Object.keys(el.input).length == 0){
			//if input is empty
			return
		}
		rd = [el['meta']['id']]
		input_header.forEach(function(v){
			val = typeof el[input_type][v] === 'undefined' ? "" : el[input_type][v]
			rd.push(val)
		})
		esyn_meta_cols.forEach(function(emc){
			rd.push(el['output'][emc])
		})
		rows.push('"' + rd.join('","') + '"\r\n')
	})
	console.log(rows)
...
// functions to position new labelnode edges

function unlock_all(){
	cy.nodes().unlock()
}

function position_node_edges(source, to_move){
	var eles = cy.elements()
	console.log(source, 'has', eles.length, 'outgoers')
	if(eles.length > 0){
		eles.lock()
		cy.$('#' + to_move).unlock()
		console.log('positioning edges')
		var bb = eles.renderedBoundingBox()
		bb = _.pick(bb, 'x1', 'y1', 'w', 'h')
		console.log('bb', bb)
		eles.layout({name:'cose',nodeDimensionsIncludeLabels: true, fit: false, stop: unlock_all}).run()
		//this should work + be faster but the edge immediately disappears
		//var move_node = cy.$('#' + to_move)
		//move_node.layout({name:'cose', boundingBox: bb,nodeDimensionsIncludeLabels: true, fit: false, stop: unlock_all}).run()
...
//send batch mode dataset to run dataset
function explore_tracking_dataset(){
	var yes = true
	if(window.dataset !== undefined) {
		if(window.dataset.data.length != 0){
			yes = confirm("This will replace your current dataset and cannot be undone. Would you like to continue?")
		}
	}
	if(yes){
		start_tracking_mode()
		window.dataset = window.batch_mode_dataset
		var next = window.dataset.previous()
		tracking_ui_update(next, window.dataset.position)
	}
}
...
function optimise_labels(){
	//lock places
	cy.nodes().lock()
	//unlock labels
	var eles = cy.nodes('.labelnode')
	eles.unlock()
	var bb = cy.nodes().renderedBoundingBox()
	bb = _.pick(bb, 'x1', 'y1', 'w', 'h')
	eles.layout({name:'cose', boundingBox: bb,nodeDimensionsIncludeLabels: true, fit: false, stop: unlock_all}).run()
}

function optimise_labels_from(n){
	//lock places
	cy.nodes().lock()
	//unlock labels
	var eles = cy.$('#' + n).neighborhood().filter('.labelnode')
	eles.unlock()

	eles.layout({name:'cose',
	numIter: 5000,
...
////////////////////////////////////////////////////////////////////////////////////////////
// conditions for decisiontree models
function buildConditionInterface(id){
    //If the edge already has an ELSE condition, can't add any more
    var conds = window.stack.metadata.conditions[id]
    var has_conditions = conds.length != 0
    var is_else = false
    if (has_conditions){
        is_else =  conds[0].operation == 'else' //else can only exist in isolation
    }

    //does the source node already have an edge with the condition ELSE
    els = cy.$('#' + id).incomers('node').outgoers('node')
    var else_from_src = els.filter(function(e){
      var c = window.stack.metadata.conditions[e.id()]
      if(c.length == 0){return false}
      else {return c[0].operation == 'else'}
      })
    var source_has_else = else_from_src.length != 0

    if (is_else) {
			//TODO show message that else can't be combined
        //print("<p>Else conditions cannot be combined with other conditions</p>")
    } else {

        var x = document.getElementById("conditionOperatorSelect");
				x.innerHTML = "" //clear current options
        var ops = ['=', '!=', '>', '>=', '<', '<=', 'else']
        if (has_conditions || source_has_else){
            ops = ['=', '!=', '>', '>=', '<', '<='] // else is only an option if there are no other conditions and the source node has no other else nodes leaving
        }
    ...
    buildConditionList(id)

}


function addConditionButton(id){
    var condition_obj = {}
    var op = $('#conditionOperatorSelect').val()
    condition_obj['operation'] = op
    condition_obj['prop'] = op == 'else' ? "" : $('#textconditionprop').val()
    condition_obj['value'] = op == 'else' ? "" : $('#textconditionvalue').val()
    condition_obj['value_type'] = op == 'else' ? "" : detectConditionValueType(condition_obj['value'])
    //the condition string is used for printing and also functions as a unique key // does it? where? There's no reason conditions should have to be unique
    condition_obj['cond_str'] = condition_obj['prop'] + " " + condition_obj['operation'] + " " + condition_obj['value']

    addCondition(condition_obj, id) // internals of adding a condition

    //buildConditionList(id)
    updateConditionLabel(id)
    //reset ui
    $('#textconditionprop').val('')
    $('#textconditionvalue').val('')

    cy.$('#' + id).trigger('vclick')
}

function addCondition(condition_obj, id){
     //check if the type of the input matches the type of the proporty if it already exists
    if( window.stack.metadata.variables.hasOwnProperty(condition_obj['prop']) ){
        var correct_type = window.stack.metadata.variables[condition_obj['prop']].value_type
        if (correct_type != condition_obj['value_type']){
            alertTypeMismatch(condition_obj['prop'], correct_type, condition_obj['value_type'])
            return false //have to stop
        } else {
            // existing variable and correct type for input so add new appearance
            if (window.stack.metadata.variables[condition_obj['prop']].appearances.hasOwnProperty(id)) {
                window.stack.metadata.variables[condition_obj['prop']].appearances[id] += 1
            } else {
                window.stack.metadata.variables[condition_obj['prop']].appearances[id] = 1
            }
        ...
        //it is a new variable name
        var variable_obj = {}
        variable_obj['value_type'] = condition_obj['value_type']
        variable_obj['required'] = false
        variable_obj['appearances'] = {}
        variable_obj['appearances'][id] = 1
        variable_obj['ui_group'] = ''
        window.stack.metadata.variables[condition_obj['prop']] = variable_obj
        window.stack.metadata.dt_ui_groups[''].push(condition_obj['prop'])
        window.autocomplete_index.add(condition_obj['prop'])
    }

    window.stack.metadata.conditions[id].push(condition_obj)
}

function ve_rename_variable(){
 var newname = $("#ve-var-rename").val()
 var oldname = $('#ve-select-var').val()
 var renamed = rename_variable(oldname, newname)
 if(renamed){
   ve_refresh()
   re_refresh()
   build_app()
   //clear ui beside graph
   clear()
 }
}

function rename_variable(oldname, newname){
  //first check if newname exists
  var exists = window.stack.metadata.variables.hasOwnProperty(newname)
  var yes = true;
  //if exists and different type, cannot merge
  if(exists){
    var t1 = window.stack.metadata.variables[newname].value_type
    var t2 = window.stack.metadata.variables[oldname].value_type
    if(t1 != t2){
      alert("Variable " +newname + " already exists with a different type and cannot be merged.")
      console.log("type mismatch:",t1, t2)
      return false
    } else {
      yes = confirm("Variable " +newname + " already exists, would you like to merge with " + oldname + "?")
      console.log("merge confirmed:",yes)
    }
  }
  if(!yes){
    return false
  ...
  var grp_oldname = window.stack.metadata.variables[oldname].ui_group
  var grp_newname
  if(exists){
    grp_newname = window.stack.metadata.variables[newname].ui_group
  }

  //TODO if exsits, merge appearances
  if(exists){
    var a_old = window.stack.metadata.variables[oldname].appearances
    var k = Object.keys(a_old)
    k.forEach(function(el){
      if(window.stack.metadata.variables[newname].appearances.hasOwnProperty(el)){
        window.stack.metadata.variables[newname].appearances[el] += 1
      } else {
        window.stack.metadata.variables[newname].appearances[el] = 1
      }
    })
  } else {
    window.stack.metadata.variables[newname] = window.stack.metadata.variables[oldname]
  }
  delete window.stack.metadata.variables[oldname]

  //newname definitely exists in metadata.variables now
  var v = window.stack.metadata.variables[newname]

  //delete old name from its group
  var idx = window.stack.metadata.dt_ui_groups[grp_oldname].indexOf(oldname)
  window.stack.metadata.dt_ui_groups[grp_oldname].splice(idx, 1)

  //adding newname to group
  //if newname already exists, nothing to do as it will be in a group
  //if newname is new, add it to group of oldname
  if(!exists){
    window.stack.metadata.dt_ui_groups[grp_oldname].push(newname)
  }

  var used_in = Object.keys(v.appearances)
  //update all conditions where this name appears
  //update all condition labels where this name appers
  used_in.forEach(function(el){
    window.stack.metadata.conditions[el].forEach(function(c){
      if(c.prop == oldname){
        c.prop = newname
        c['cond_str'] = c['prop'] + " " + c['operation'] + " " + c['value']
      }
    })
  ...
  //Bloodhound
  //don't bother removing the old name
  window.autocomplete_index.add(newname)

  //calculators/rules??
  //todo change variable name in function str for if_str and then_str
  //MUST be an exact match not substring
  var rules = Object.keys(window.stack.metadata.dt_rules)
  //separate out set-val and not-valid rules
  var set_val_rules = []
  var not_valid_rules = []
  var then_action, r, if_fn, then_fn
  //make regex
  var reg = new RegExp("'" + oldname + "'")
  var newname_quoted = "'" + newname + "'"
  rules.forEach(function(el){
      r = then_action = window.stack.metadata.dt_rules[el]
      then_action = r.then_str
      if(then_action == 'set-val'){
          set_val_rules.push(el)
      }
      else if(then_action == 'not-valid'){
      ...
      //if fn
      if(reg.test(r.if_str)){
        //console.log("replacing", oldname,"in IF for rule",el)
        var new_if = r.if_str.replace(reg, newname_quoted)
        r.if_str = new_if
        r['if'] = compileExpression(new_if);
      }


  })

  //for set val rules aka calculators, change the name of their output
  var ts
  set_val_rules.forEach(function(el){
    if(window.stack.metadata.dt_rules[el].then.set_variable == oldname){
      window.stack.metadata.dt_rules[el].then.set_variable = newname
    }
    //then fn
    ts = window.stack.metadata.dt_rules[el].then.str
    if(reg.test(ts)){
      //console.log("replacing", oldname,"in THEN for rule",el)
      var new_then = ts.replace(reg, newname_quoted)
      window.stack.metadata.dt_rules[el].then.str = new_then
      window.stack.metadata.dt_rules[el].then.fn = compileExpression(new_then);
    }
  })
...
function check_else_condition(id){
    //no property or value set

    //no other conditions for this edge

    //
}

function detectConditionValueType(value){
    var as_float = parseFloat(value)
    bool_opts = get_bool_opts()
    if (isNaN(as_float)){
        if( bool_opts.indexOf(value) != -1){
            return 'bool'
        } else {
            return 'str'
        }
...
//helper so these options only have to be defined once
function get_bool_opts(){
    var tr = get_true_opts()
    var fl = get_false_opts()
    return tr.concat(fl)
}

function get_true_opts(){
    return ['TRUE', 'True', 'true']
}
function get_false_opts(){
    return ['FALSE', 'False', 'false']
}

//deleting a condition through UI
function removeConditionButton(id){
    var selected = $('#selectcondition').val()

    removeCondition(id, selected)

    updateConditionLabel(id)
    cy.$('#' + id).trigger('vclick')
}

//internals of deleting a condition
function removeCondition(id, selected){
    var keep = []
    window.stack.metadata.conditions[id].forEach(function(el){
        if(selected.indexOf(el.cond_str) != -1){
            //get the ui group first before (potentially) deleting the variable
            var uig = window.stack.metadata.variables[el.prop].ui_group

            //delete
            window.stack.metadata.variables[el.prop].appearances[id] -= 1
            if(window.stack.metadata.variables[el.prop].appearances[id] == 0){
                delete window.stack.metadata.variables[el.prop].appearances[id]
                if(Object.keys(window.stack.metadata.variables[el.prop].appearances).length == 0){
                    //there are no more appearances of this variable so delete it from metadata
                    //variable tracking
                    delete window.stack.metadata.variables[el.prop]
                    //ui groups
                    var pos = window.stack.metadata.dt_ui_groups[uig].indexOf(el.prop)
                    window.stack.metadata.dt_ui_groups[uig].splice(pos, 1)
                }
            }
        } else {
            keep.push(el)
    ...
    window.stack.metadata.conditions[id] = keep
}

function buildConditionList(id){
    $('#selectcondition').empty()
    var x = document.getElementById("selectcondition");

		var has_else = false;
    window.stack.metadata.conditions[id].forEach(function(el){
        cond_str = el.cond_str
        var option = document.createElement("option");
        option.text = cond_str;
        x.add(option);
				if(el.operation == 'else'){
					has_else = true;
				}
    })
		$('#buttonaddcondition').prop('disabled',false)
		if(has_else){
			$('#buttonaddcondition').prop('disabled',true)
		}
}
...
function updateConditionLabel(id){
    ...
    window.stack.metadata.conditions[id].forEach(function(el){
        cond_str = el.cond_str
        label = label + cond_str + "\n"
    })
    label = label.slice(0, -1) // remove trailing chars
    edge.data('conditions_label', label)
}

function alertTypeMismatch(name, correct, entered){
    var alertmsg = "Error: The variable " + name + " is of type " + correct + " but a " + entered + " was entered for the condition."
    alert(alertmsg)
}
...
//variable editor UI
////////////////////////////////////////////////////////////////////////////////////////////
function list_variables_in(select_elem){
    var varselect = $('#' + select_elem)
    var varOpts = '<option selected="selected" value="">Select a variable</option>'
    var vnames = Object.keys(window.stack.metadata.variables)
    vnames.forEach(function(val){
        bit = '<option value ="' + val + '">' + val + '</option>'
        varOpts += bit
    });

    varselect.html(varOpts)
}

function ve_refresh(){
    //update the dropdown list of options
   list_variables_in('ve-select-var')

    ve_reset_ui()
    ve_build_group_list()
    ve_show_contained_vars('')
}

function ve_reset_ui(){
    //clear any data from the ui
    document.getElementById('ve-required').checked = false
    $('#ve-selected').text('Select a variable to edit')
    $('#ve-var-rename').val('')

}

function ve_display_variable(){
    var vname = $('#ve-select-var').val()
    var vtype = window.stack.metadata.variables[vname].value_type
    var vgroup = window.stack.metadata.variables[vname].ui_group
    $('#ve-selected').text('Editing: ' + vname + " (" + vtype + ")")
    $('#ve-current-group').text('Current group: ' + vgroup)
    document.getElementById('ve-required').checked = window.stack.metadata.variables[vname].required //safe because onchange won't trigger and anyway would be triggered by the correct value

    var desc = ""
    if(window.stack.metadata.variables[vname].hasOwnProperty('description')){
      desc = window.stack.metadata.variables[vname].description
    }
    $('#ve_var_description').val(desc)

}

function ve_display_group(){
    var grpname = $('#ve-var-group-edit').val()
    $('#ve-grp-selected').text('Editing group: ' + grpname )
    ve_show_contained_vars(grpname)
    $('#ve-group-rename').val('') //clear rename box
}

function ve_show_contained_vars(grp){
    $('#ve-grp-contains').empty()
    var x = document.getElementById("ve-grp-contains");

    window.stack.metadata.dt_ui_groups[grp].forEach(function(el){
        var option = document.createElement("option");
        option.text = el;
        x.add(option);
    })
...
function ve_set_required(){
    var vname = $('#ve-select-var').val()
    window.stack.metadata.variables[vname].required = document.getElementById('ve-required').checked
}

//handles internals of changing a group
function set_dt_ui_group(variable, group){
    //remove variable from old group
    //have to do this first or grp_old will change to the new group
    var grp_old = window.stack.metadata.variables[variable].ui_group
    var idx = window.stack.metadata.dt_ui_groups[grp_old].indexOf(variable)
    console.log('removing',variable,'from old group',grp_old)
    window.stack.metadata.dt_ui_groups[grp_old].splice(idx, 1)

    //add variable to new group
    if(!window.stack.metadata.dt_ui_groups.hasOwnProperty(group)){
        window.stack.metadata.dt_ui_groups[group] = [variable]
        console.log('added', variable, 'to new group', group)
    } else {
        if(window.stack.metadata.dt_ui_groups[group].indexOf(variable) == -1){
            window.stack.metadata.dt_ui_groups[group].push(variable)
            console.log('added', variable, 'to existing group', group)
        } else {
            console.log('variable', variable, 'is already in group', group)
        }

    }

    //update the variable itself
    window.stack.metadata.variables[variable].ui_group = group
}

//handles ui of changing a group
function ve_set_group(){
    var selected_grp = $('#ve-var-group').val()
    var vname = $('#ve-select-var').val()
    set_dt_ui_group(vname, selected_grp)
    $('#ve-current-group').text('Current group: ' + selected_grp)
}

//change variable description
function ve_set_description(){
  var vname = $('#ve-select-var').val()
  var desc = $("#ve_var_description").val()
  window.stack.metadata.variables[vname].description = desc
}

//update dropdown list of current variable groups
function ve_build_group_list(){
    var vname = $('#ve-select-var').val()
    var varselect = $('#ve-var-group')
    var otherselect = $('#ve-var-group-edit')
    var grpnames = Object.keys(window.stack.metadata.dt_ui_groups)
    var varOpts = ''
    grpnames.forEach(function(val){
        if(val==""){
          bit = '<option value ="' + val + '">(none)</option>'
        }
        else{
          bit = '<option value ="' + val + '">' + val + '</option>'
        }
    ...
    varselect.html(varOpts)
    otherselect.html(varOpts)
}

function ve_create_new_group(){
    var grpname = $('#ve-new-group-name').val()
    if(window.stack.metadata.dt_ui_groups.hasOwnProperty(grpname)){
        alert("This group already exists")
    } else {
        window.stack.metadata.dt_ui_groups[grpname] = []
        ve_build_group_list()
        $('#ve-new-group-name').val('')
    }
}

//delete is just moving contents to default group
function ve_delete_group(){
    var selected = $('#ve-var-group-edit').val()
    //put all variables from this group back into the default group
    var new_grp = ""
    ve_move_group_members(selected, new_grp)
    ve_build_group_list()
    $('#ve-var-group-edit').val(new_grp).change()
}

function ve_rename_group(){
    //create new group if it doesn't exist
    var old_grp = $('#ve-var-group-edit').val()
    var new_grp = $('#ve-group-rename').val()
    if(!window.stack.metadata.dt_ui_groups.hasOwnProperty(new_grp)){
        window.stack.metadata.dt_ui_groups[new_grp] = []
    }
    //put all members of old group into new one
    ve_move_group_members(old_grp, new_grp)
    ve_build_group_list()

    //select the new group name and trigger events
    $('#ve-var-group-edit').val(new_grp).change()
}

//used to rename or delete (special case of rename)
function ve_move_group_members(from, to){
    if(from == to){
      return
    }
    var contained = window.stack.metadata.dt_ui_groups[from]
    contained.forEach(function(el){
        set_dt_ui_group(el, to)
    })
    delete window.stack.metadata.dt_ui_groups[from]
}

function ve_remove_from_group(){
    var selected = $('#ve-grp-contains').val()
...
//get params from ui and create a new rule
function re_create_rule(){
    var rule = {}
    rule['if_str'] = $('#re-rule-string').val()
    rule['then_str'] = $('#re-then').val()
    rule['name'] = $('#re-rule-name').val()
    rule['then'] = {}

    if(rule['then_str'] == 'set-val'){
        rule['then']['str'] = $('#re-rule-value').val()
        rule['then']['set_variable'] = $('#re-set-val-of').val()
    }
...
function add_rule(rule){
    ...
    if(rule['then_str'] == 'not-valid') {
        rule['then']['fn'] = rule_input_invalid
    }
    //update metadata
    window.stack.metadata.dt_rules[rule['name']] = rule
    console.log('rule created')
}

function delete_rule(name){
    delete window.stack.metadata.dt_rules[name]
}

function re_delete_rule(name){
    delete_rule(name)
    re_update_rule_list()
}

function re_edit_rule(name){
    var rule = window.stack.metadata.dt_rules[name]
    $('#re-rule-string').val(rule['if_str'])
    $('#re-then').val(rule['then_str']).change()
    $('#re-rule-name').val(name)

    if(rule['then_str'] == 'set-val'){
         $('#re-rule-value').val(rule['then']['str'])
        $('#re-set-val-of').val(rule['then']['set_variable'])
    }
}

function re_copy_rule(name){
    //set rule UI based on existing rule then change the name
    re_edit_rule(name)
    $('#re-rule-name').val(name + ' (copy)')

}

function rule_input_invalid(){
    return true;
}

function get_rules_by_type(){
    var rules = Object.keys(window.stack.metadata.dt_rules)
    var set_val_rules = []
    var not_valid_rules = []
    var then_action
...
function run_all_calculators(user_input){
    rules = get_rules_by_type()
    calculated = run_calculators(rules.set_val_rules, user_input)
    return calculated
}


// determine wither the model can run after applying calculators
// does not affect user input
// user input is returned with calculated values included as 'updated_input'
function run_rules(user_input){
    var then_action
    var log = []
    var can_run_model = true
    ...
    rules.not_valid_rules.forEach(function(el){
        var do_then = window.stack.metadata.dt_rules[el].if(calculated)
        if(do_then == 1){
            can_run_model = false
            not_valid_rules_state[el] = true
    ...
    var calculated_vars = _.keys(calculated)
    var updated_input = _.clone(user_input)
    calculated_vars.forEach(function(el){
        updated_input[el] = calculated[el]
    })
    var output = {log:log, can_run:can_run_model, calculated: calculated, updated_input: updated_input}
    return output
}

// ui to list and delete rules
function re_update_rule_list(){
    init_rule_list_ui()
    var tbl = document.getElementById('re-active-rule-list')
    var all_rules = _.keys(window.stack.metadata.dt_rules).sort()
    all_rules.forEach(function(gr){
        add_rule_to_list(window.stack.metadata.dt_rules[gr])
    })

}

function add_rule_to_list(rule_data){
    var tbl = document.getElementById('re-active-rule-list')
    var row = tbl.insertRow(-1) //position 0 will insert above the header

    var c0 = row.insertCell(0)
    c0.innerText = rule_data.name

    var if_str = rule_data['if_str']
    var c1 = row.insertCell(1)
    c1.innerText = if_str

    var cell_txt;
    //triggers can be oninput to evaluate per keystroke but creates problems e.g. if rule triggers at 12 and user is typing 120, rule will run after 12 and could set various values
    if(rule_data['then'].hasOwnProperty('str')){
        cell_txt = rule_data['then']['set_variable'] + " = " + rule_data['then']['str']
    } else{
        cell_txt = "Input not valid"
    }

    var c2 = row.insertCell(2)
    c2.innerText = cell_txt

    var c3 = row.insertCell(3)
    var rule_name = rule_data.name
    var rule_name_escaped = rule_name.replace(/'/g, '\\\'').replace(/"/g, '\\\"')
    var btns = '<button class="btn btn-primary btn-sm" onclick="re_edit_rule('+"'"+rule_name_escaped +"'"+')">Edit</button>&nbsp;'
    btns += '<button class="btn btn-info btn-sm" onclick="re_copy_rule('+"'"+rule_name_escaped +"'"+')">Copy</button>&nbsp;'
    btns += '<button class="btn btn-danger btn-sm" onclick="re_delete_rule('+"'"+rule_name_escaped +"'"+')">Delete</button>'
    c3.innerHTML = btns
}

function init_rule_list_ui(){
    var tbl = ''
    tbl += '<thead><tr><th>Rule</th><th>Condition</th><th>Output</th><th>Options</th></tr></thead>'
    $('#re-active-rule-list').html(tbl)
}

/*
In any one run of all calculators, they are completely isolated from each other
the output of one calculator CANNOT be the input to another in any single run
because outputs are stored in {calculated} and input is in {user_input}.
For now this is required to prevent accidental use of intermediate calculated values
(calculated because all rules are run then output is blocked) for variables  that
should not be touched by calculators because they have been set by the user.
*/
function run_calculators(set_val_rules, user_input){
    var calculated = {}
    var r
    set_val_rules.forEach(function(el){
        //check the IF condition
        var do_then = window.stack.metadata.dt_rules[el].if(user_input)
        if(do_then == 1){
...
function re_refresh(){
    list_variables_in('re-set-val-of')
    re_update_rule_list()
}

function re_reset_fields(){
    $('#re-rule-string').val('')
    $('#re-rule-name').val('')
    $('#re-rule-value').val('')
    $('#re-set-val-of').val('')
...
function re_select_then(){
    var then = $('#re-then').val()
    if(then == 'set-val'){
        $('#re-set-val-of-container').show()
    } else {
        $('#re-set-val-of-container').hide()
    }
}

//adds some conditions so variables exist in stack for testing without clicking through UI
function _build_test(){
    ...
    window.stack.metadata.conditions[dummy] = []

//two numbers for testing rules
    cy.$('#_e0').emit('tap')
    $('#textconditionprop').val('a')
    $('#conditionOperatorSelect').val('=')
    $('#textconditionvalue').val('5')
    addConditionButton('_e0')
    $('#textconditionprop').val('b')
    $('#conditionOperatorSelect').val('=')
    $('#textconditionvalue').val('5')
    addConditionButton('_e0')

    //a string
    $('#textconditionprop').val('string')
    $('#conditionOperatorSelect').val('=')
    $('#textconditionvalue').val('test')
    addConditionButton('_e0')

    //a bool
    $('#textconditionprop').val('bool')
    $('#conditionOperatorSelect').val('=')
    $('#textconditionvalue').val('true')
    addConditionButton('_e0')

    //
    // var condition_obj = {}
    // condition_obj['operation'] = '='
    // condition_obj['prop'] = 'a'
    // condition_obj['value'] = 5
    // condition_obj['value_type'] = detectConditionValueType(condition_obj['value'])
    // //the condition string is used for printing and also functions as a unique key // does it? where? There's no reason conditions should have to be unique
    // condition_obj['cond_str'] = condition_obj['prop'] + " " + condition_obj['operation'] + " " + condition_obj['value']

    // addCondition(condition_obj, dummy) // internals of adding a condition
    // condition_obj['prop'] = 'b'
    // condition_obj['cond_str'] = condition_obj['prop'] + " " + condition_obj['operation'] + " " + condition_obj['value']
    // addCondition(condition_obj, dummy)
}


function _fix_ui_groups(){
    //check groups only refer to conditions that exist
    var all_vars = Object.keys(window.stack.metadata.variables)
    var grps = Object.keys(window.stack.metadata.dt_ui_groups)
    grps.forEach(function(gr){
        var contained = window.stack.metadata.dt_ui_groups[gr]
        var keep = _.intersection(contained, all_vars)
        window.stack.metadata.dt_ui_groups[gr] = keep
    })

    //remove any variables from the default group that appear in user-defined groups
    var grps = Object.keys(window.stack.metadata.dt_ui_groups)
    grps.splice(grps.indexOf(''),1)
    var in_default = window.stack.metadata.dt_ui_groups['']
    grps.forEach(function(gr){
        var contained = window.stack.metadata.dt_ui_groups[gr]
        in_default = _.difference(in_default, contained)
    })
    window.stack.metadata.dt_ui_groups[''] = in_default
}

function _fix_appearances(){
    //for every variable, find all current appearances and update the variable metadata
    var edges = cy.edges()
    var keep_ids = []
    edges.forEach(function(edge){
        keep_ids.push( edge.data('id'))
    })
    var deleted_vars = []
    var vars = Object.keys(window.stack.metadata.variables)
    vars.forEach(function(el){
        var apps = Object.keys(window.stack.metadata.variables[el].appearances)
        apps.forEach(function(ap){
            if (keep_ids.indexOf(ap) == -1){
                delete window.stack.metadata.variables[el].appearances[ap]
            }
        })
        //now delete variables with no appearances
        var apps_now = Object.keys(window.stack.metadata.variables[el].appearances)
        if (apps_now.length == 0){
            deleted_vars.push(el)
            delete window.stack.metadata.variables[el]
        }

    })
    //if any variables were deleted, remove from UI groups
    //don't delete UI groups even if empty


    //now fix edge metadata
    var cond_eids = Object.keys(window.stack.metadata.conditions)
    cond_eids.forEach(function(el){
        if(keep_ids.indexOf(el) == -1){
            delete window.stack.metadata.conditions[el]
        }
    })

}

function _run_dt_fixes(){
    _fix_appearances()
    _fix_ui_groups()
